...
// Servicio completo: Hornet compra y envía
const FEE_PARTICULAR = 0.15;
const FEE_MAYORISTA  = 0.12;

// Forwarding: cliente ya compró, Hornet solo hace Miami → BsAs
const FEE_FORWARDING_PARTICULAR = 0.08;
const FEE_FORWARDING_MAYORISTA  = 0.06;

const IVA_RATIO               = 0.21;
const TASA_ESTADISTICA_RATIO  = 0.03;
const PRECIO_MINIMO_PARTICULAR = 25;
const PRECIO_MINIMO_MAYORISTA  = 200;
const PRECIO_MINIMO_FORWARDING = 10;
...
export function calcularCotizacion(
  ...
  // Mínimos diferenciados por modo
  if (esForwarding) {
    if (input.precioUsdProducto < PRECIO_MINIMO_FORWARDING)
      return { ok: false, razon: "precio_minimo" };
  } else {
    const minimo = esMayorista ? PRECIO_MINIMO_MAYORISTA : PRECIO_MINIMO_PARTICULAR;
    if (input.precioUsdProducto < minimo)
      return { ok: false, razon: esMayorista ? "precio_minimo_mayorista" : "precio_minimo" };
  }

  if (input.pesoKg <= 0 || input.pesoKg > PESO_MAX_KG)
    return { ok: false, razon: "peso_excedido" };

  const feeRatio = esForwarding
    ? (esMayorista ? FEE_FORWARDING_MAYORISTA  : FEE_FORWARDING_PARTICULAR)
    : (esMayorista ? FEE_MAYORISTA             : FEE_PARTICULAR);

  const pesoFacturable     = redondearAlMedio(input.pesoKg);
  const costoFlete         = pesoFacturable * TARIFA_FLETE_USD_KG;

  // CIF = precio declarado + flete (base para aranceles en ambos modos)
  ...
  // En forwarding el cliente ya pagó el producto → total solo incluye logística
  const total = esForwarding
